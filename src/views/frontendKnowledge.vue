...
// 获取封面图片
const getImage = (url) => {
  if (!url || url === 'a') return 'https://file.itndedu.com/psychology_ai.png'
  if (url.startsWith('http')) return url
  if (url.startsWith('/')) return 'http://159.75.169.224:1235' + url
...
onMounted(() => {
  ...
  getKnowledgeList(params).then(res => {
    console.log(res)
    recommendList.value = res.records || res.data || res
  })
})
</script>
