...
const initEmotionChart = () => {
  ...
  // console.log('=== 图表初始化检查 ===')
  // console.log('1. 情绪趋势数据:', emotionTrend)
  // console.log('2. 数据是否存在:', !!emotionTrend)
  // console.log('3. 数据长度:', emotionTrend?.length || 0)

  // if (!emotionTrend || !emotionTrend.length) {
  //   console.log('❌ 没有情绪趋势数据')
  //   return
  // }

  // console.log('4. 第一个数据项:', emotionTrend[0])
  // console.log('5. 包含的字段:', emotionTrend[0] ? Object.keys(emotionTrend[0]) : '无')

  // // 打印所有字段的值
  // if (emotionTrend[0]) {
  //   console.log('=== 第一个数据项的所有字段值 ===')
  //   Object.entries(emotionTrend[0]).forEach(([key, value]) => {
  //     console.log(`${key}:`, value)
  //   })
  // }
  // console.log('6. 是否有date字段:', emotionTrend[0]?.date !== undefined)
  // console.log('7. 是否有avgMoodScore字段:', emotionTrend[0]?.avgMoodScore !== undefined)
  // console.log('8. 是否有count字段:', emotionTrend[0]?.count !== undefined)
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      data: emotionTrend.map(item => item.date),
      axisLine: {
        lineStyle: {
          color: '#2d3436'
        }
    ...
    series: [{
      name: '平均情绪评分',
      type: 'line',
      data: emotionTrend.map(item => item.avgMoodScore),
      smooth: true,
      lineStyle: {
        width: 3,
        color: '#faebaf'
      ...
      type: 'line',
      yAxisIndex: 1,
      data: emotionTrend.map(item => item.recordCount),
      smooth: true,
      lineStyle: {
        width: 3,
        color: '#fab1a0'
...
onMounted(() => {
  getAnalyticsOverview().then(res => {
    ...
    // console.log('Emotion trend data:', res.emotionTrend)
    initCharts()
